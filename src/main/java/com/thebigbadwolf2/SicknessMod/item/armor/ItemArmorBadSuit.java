...
public class ItemArmorBadSuit extends ItemArmorWSMod
{
	public ItemArmorBadSuit(int type)
	{
		super(ItemArmorWSMod.badSuit,1,type);
	...
	public void onArmorTick(World world, EntityPlayer player, ItemStack itemStack) {
		...
		if (this.armorType == 1){
			...
			for (int i = -25; i <= 25; i++)
				for (int j = -15; j <= 5; j++)
					for (int k = -25; k <= 25; k++)
						MethodRef.Bad.Convert.convert(world, x + i, y + j, z + k,true);
		}
